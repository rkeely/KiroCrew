...
| Pinned prompt banner | Keeps the user's latest typed prompt — a steer included — visible in a single-state banner at the top of a long reply while it scrolls; distinct from **Pinned messages** above. Clicking it glides back to that prompt, smooth at any distance and landing exactly, then walks one turn further back per click. Worn by the main chat AND by every `ChatPane` host — split-view panes and the Crew Members DM thread — through the same hook | Settings → Chat → **Pin the latest turn** | `pages/chat/PinnedPrompt.tsx`, `pages/chat/usePinnedPrompt.ts` (internal split of `useChatPageTranscriptController` — the host-agnostic pin geometry, scroll recompute and in-place jump shared with `components/ChatPane.tsx`; no new user-facing entry point), `utils/pinnedPrompt.ts` (which rows pin, push and anchor the jump), `utils/convergingGlide.ts` (the self-driven travel-then-converge scroll both jump hosts run; the virtualized host steers an unmounted target by `useVirtualChat.estimateRowTop`), `app-sdk/ChatMessageList.tsx` (`onDisplayItems` / `hiddenRow` row-indexing seam) | client-only (no handler) | none |
